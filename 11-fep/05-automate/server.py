...
def loop():
    ...
    too_many_jobs = False
    if "Not Running: would exceed user z5358697's limit" in qstat_process.stdout.decode('utf-8'):
        too_many_jobs = True
    ...
    for job_id, v in jobs.items():
        ...
        print(f"{job_id.split('.')[0]: <8} {v['Job_Name']: <20} {v['job_state']}")
        match v['job_state']:
            case 'R':
                running_job_int += 1
            case 'Q':
            ...
            case x:
                ...
                sys.exit(1)
    print()
    print(f"{running_job_int} jobs running, {queuing_job_int} jobs queuing, {exiting_job_int} jobs exiting")
    ...
    server_ls_process = subprocess.run(['ssh', 'katana', 'ls /srv/scratch/z5358697/automated'], capture_output=True)
    ...
    subprocess.run(['scp', '-r', prefix, 'katana:/srv/scratch/z5358697/automated/']).check_returncode()

    print(f"submitting job")
    subprocess.run(['ssh', 'katana', f'cd /srv/scratch/z5358697/automated/{prefix} && qsub {prefix}_equilibrate']).check_returncode()
