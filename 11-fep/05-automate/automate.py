...
gmx_top = pmd.load_file(f'FreeSolv/gromacs_solvated/mobley_{prefix}.top', xyz=f'FreeSolv/gromacs_solvated/mobley_{prefix}.gro')
gmx_top.save(f'{prefix}/mobley_{prefix}.psf')
gmx_top.save(f'{prefix}/mobley_{prefix}.pdb')

logger.info(f"Copying prm file")
subprocess.run(['cp', f'FreeSolv/charmm/mobley_{prefix}.prm', f'{prefix}/mobley_{prefix}.prm']).check_returncode()

logger.info(f"Fixing mobley_{prefix}.psf")
subprocess.run(['sed', '-i', f'4 c \\ REMARKS mobley_{prefix}/', f'{prefix}/mobley_{prefix}.psf'])

logger.info(f"Fixing mobley_{prefix}.pdb")
subprocess.run(['sed', '-i', '/W/!s/0\\.00[[:space:]]\\+0\\.00/0.00  1.00/g', f'{prefix}/mobley_{prefix}.pdb'])

logger.info("Reading coordinates")
with open(f"{prefix}/mobley_{prefix}.pdb") as f:
    line = f.readline()
    origin_x, origin_y, origin_z, length_x, length_y, length_z = map(float, line.split()[1:7])
...
with open(f"{prefix}/{prefix}_equilibrate", 'w') as f:
    ...
    f.write('PATH="/srv/scratch/z5358697/namd:$PATH" namd3 +p$(nproc) equilibrate.namd || exit 2\n')
    f.write(f'qsub {prefix}_forwards\n')
    f.write(f'qsub {prefix}_backwards\n')
with open(f"{prefix}/{prefix}_forwards", 'w') as f:
    f.write(run_template)
    f.write('PATH="/srv/scratch/z5358697/namd:$PATH" namd3 +p$(nproc) forwards.namd\n')
with open(f"{prefix}/{prefix}_backwards", 'w') as f:
    f.write(run_template)
    f.write('PATH="/srv/scratch/z5358697/namd:$PATH" namd3 +p$(nproc) backwards.namd\n')
...
set outputname     mobley_{prefix}_forwards
...
alchCol                 B
alchOutFile             mobley_{prefix}_forwards.fepout
alchOutFreq             10
