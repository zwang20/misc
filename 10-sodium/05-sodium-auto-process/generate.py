...
for time_ns in time_ns_list:
    ...
    os.system(f"mkdir -p '{folder_prefix}/{folder_prefix}_{time_ns}'")

    print("copying files")
    os.system(f"cp ionized.pdb '{folder_prefix}/{folder_prefix}_{time_ns}/ionized.pdb'")
    os.system(f"cp ionized.psf '{folder_prefix}/{folder_prefix}_{time_ns}/ionized.psf'")
    os.system(f"cp par_all27_prot_lipid.inp '{folder_prefix}/{folder_prefix}_{time_ns}/'")
...
"""
    ...
    with open(f"{folder_prefix}/{folder_prefix}_{time_ns}/sodium.namd", 'w') as f:
        f.write(sodium_namd_template)

print("writing sub script")
